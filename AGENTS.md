...
- The callbacks own inbound flow. `push` and `close` are safe from other processes because they send through the socket actor, but callbacks must not call them on their own channel; return `stop` only for local runtime termination.
